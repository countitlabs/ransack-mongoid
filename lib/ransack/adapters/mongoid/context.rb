  ...
  module Adapters
    module Mongoid
      class Context < ::Ransack::Context
        ...
        def attribute_method?(str, klass = @klass)
          ...
          if ransackable_attribute?(str, klass)
            ...
            while !found_assoc && remainder.unshift(
              ...
              next unless found_assoc = get_association(assoc, klass)
        ...
        def klassify(obj)
          if obj.is_a?(Class) && obj.ancestors.include?(::Mongoid::Document)
            obj
        ...
        def get_parent_and_attribute_name(str, parent = @base)
          ...
          if ransackable_attribute?(str, klassify(parent))
            ...
            while remainder.unshift(
              segments.pop
            ) && segments.size > 0 && !found_assoc
              assoc, klass = unpolymorphize_association(segments.join('_'))
              next unless found_assoc = get_association(assoc, parent)
